document.addEventListener("DOMContentLoaded", function() {
    // Add GoatCounter script to the page
    const goatScript = document.createElement('script');
    goatScript.setAttribute('data-goatcounter', 'https://tdc.goatcounter.com/count');
    goatScript.setAttribute('async', 'true');
    goatScript.src = '//gc.zgo.at/count.js';
    document.head.appendChild(goatScript);

    // Helper function to categorize prices into tiers
    ...
    if (isDetailPage) {
        ...
        function createGridItem(domain) {
            ...
            a.href = `https://${domain.domainName}`;
            a.target = "_blank"; // Open in new tab
            a.className = "domain-item";
            
            // Add click tracking with GoatCounter
            a.addEventListener('click', function(e) {
                if (window.goatcounter && typeof window.goatcounter.count === 'function') {
                    // Get price tier for categorization
                    const priceTier = getPriceTier(domain.price);
                    
                    // Create a path that includes all tracking dimensions
                    const path = `click/grid/${domain.tld}/${priceTier}/${domain.domainName}`;
                    
                    window.goatcounter.count({
                        path: path,
                        title: `Domain: ${domain.domainName} (Grid View)`,
                        event: true
                    });
                    
                    console.log('Tracked click:', path);
                }
            });
        ...
        function createListItem(domain) {
            ...
            a.addEventListener('click', function(e) {
                if (window.goatcounter && typeof window.goatcounter.count === 'function') {
                    ...
                    // Create a path that includes all tracking dimensions
                    const path = `click/list/${domain.tld}/${priceTier}/${domain.domainName}`;
                    
                    window.goatcounter.count({
                        path: path,
                        title: `Domain: ${domain.domainName} (List View)`,
                        event: true
                    });
